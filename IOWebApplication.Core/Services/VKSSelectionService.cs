...
namespace IOWebApplication.Core.Services
{
    public class VKSSelectionService : BaseService, IVKSSelectionService
    {
        ...
        ILogger<VKSSelectionService> _logger,
        IRepository _repo,
        ...
        public List<SelectListItem> GetDDL_SelectionYear()
        {
            ...
            var selectListItems = selectListYears.Select(x => x.Value).Distinct()
        ...
        public bool Already_Exist(VksSelectionHeaderVM model)
        {
            ...
            .Where(x => x.PeriodNo == model.PeriodNo).Any());
            }
            catch (Exception)
            {
        ...
        private int[] RandomizeLawUnitsArray(ICollection<VksSelectionLawunit> selectionLawUnitsColection, bool chairman_in)
        {
            ...
            selectionLawUnits = selectionLawUnits
                               ...
                               .Where(x => (x.DateEnd ?? dateNow) >= dateNow)
                                .Where(x => (x.DateExpired ?? dateNow) >= dateNow)
                                .Where(x => x.JudgeDepartmentRoleId != NomenclatureConstants.JudgeDepartmentRole.Predsedatel) //Да не взема председателите
                                .ToList();
        ...
        private bool GenerateSelectionMonthStaff(VksSelection selection, int month)
        {
            ...
            try
            {
                ...
                //Добавяне на председател на отделение
                var PredsedatelOtdelenie = selection.SelectionLawunit
                                    .Where(x => x.JudgeDepartmentRoleId == NomenclatureConstants.JudgeDepartmentRole.Predsedatel)
                                    .Where(x => x.CourtDepartmentTypeId == NomenclatureConstants.DepartmentType.Otdelenie)
                                     .FirstOrDefault();
                if (PredsedatelOtdelenie != null)
                {


                    for (int i = 1; i < 3; i++)
                    {
                        var ThreeMemberDays = selection.Months.Where(x => x.SelectionMonth == month)
                                                        .Where(x => x.SelectionMonthLawunit.Count() < 4).Select(x => x.Id).ToArray();


                        System.Random rnd = new System.Random();
                        int index = rnd.Next(ThreeMemberDays.Length - 1);
                        var mpo = selection.Months.Where(x => x.Id == ThreeMemberDays[index]).FirstOrDefault();

                        VksSelectionMonthLawunit selectionMonthLawunit = new VksSelectionMonthLawunit();
                        selectionMonthLawunit.VksSelectionMonthId = mpo.Id;
                        var lu = repo.GetById<VksSelectionLawunit>(PredsedatelOtdelenie.Id);
                        selectionMonthLawunit.VksSelectionLawunitId = PredsedatelOtdelenie.Id;
                        selectionMonthLawunit.LawunitKey = lu.LawunitKey;
                        selectionMonthLawunit.UserId = userContext.UserId;
                        selectionMonthLawunit.DateWrt = dateNow;
                        repo.Add<VksSelectionMonthLawunit>(selectionMonthLawunit);
                        repo.SaveChanges();
                        mpo.SelectionHash = repo.AllReadonly<VksSelectionMonthLawunit>().Where(x => x.VksSelectionMonthId == mpo.Id).Select(x => x.VksSelectionLawunit.LawunitId ?? 0).OrderBy(x => x).ToArray().ConcatenateWithSeparator();
                        repo.Update<VksSelectionMonth>(mpo);
                        repo.SaveChanges();
                ...
                //Добавяне на председател на Колегия
                if (selection.Months.Where(x => x.SelectionMonth == month).FirstOrDefault().ChairmanIn == true)
                {


                    var PredsedatelKolegia = selection.SelectionLawunit
                                        .Where(x => x.JudgeDepartmentRoleId == NomenclatureConstants.JudgeDepartmentRole.Predsedatel)
                                        .Where(x => x.CourtDepartmentTypeId == NomenclatureConstants.DepartmentType.Kolegia)
                                         .FirstOrDefault();
                    if (PredsedatelKolegia != null)
                    {
                        ...
                        for (int i = 1; i < 3; i++)
                        {
                            ...
                            System.Random rnd = new System.Random();
                            int index = rnd.Next(ThreeMemberDays.Length - 1);
                            var mpc = selection.Months.Where(x => x.Id == ThreeMemberDays[index]).FirstOrDefault();

                            VksSelectionMonthLawunit selectionMonthLawunit = new VksSelectionMonthLawunit();
                            selectionMonthLawunit.VksSelectionMonthId = mpc.Id;
                            var lu = repo.GetById<VksSelectionLawunit>(PredsedatelKolegia.Id);
                            selectionMonthLawunit.VksSelectionLawunitId = PredsedatelKolegia.Id;
                            selectionMonthLawunit.LawunitKey = lu.LawunitKey;
                            selectionMonthLawunit.UserId = userContext.UserId;
                            selectionMonthLawunit.DateWrt = dateNow;
                            repo.Add<VksSelectionMonthLawunit>(selectionMonthLawunit);
                            repo.SaveChanges();
                            mpc.SelectionHash = repo.AllReadonly<VksSelectionMonthLawunit>().Where(x => x.VksSelectionMonthId == mpc.Id).Select(x => x.VksSelectionLawunit.LawunitId ?? 0).OrderBy(x => x).ToArray().ConcatenateWithSeparator();
                            repo.Update<VksSelectionMonth>(mpc);
                            repo.SaveChanges();
        ...
        public VksSelectionCalendarVM GetSelectionCalendar(int selection_id)
        {
            ...
            foreach (var month in mnt)
            {
                ...
                oneMonthList.SelectionMonthString = NomenclatureExtensions.IntToMonth(month);
...

                {
                    ...
                    foreach (var month_lawunit in selestion_month.SelectionMonthLawunit)
                    {
                        ...
                        if (month_lawunit.VksSelectionLawunit.IsUnknownJudge)
                        {
                            ...
                            month_lawunit.VksSelectionLawunit.OrderNumber = int.MaxValue - 1;
                            continue;
                        }
                        ...
                        if (month_lawunit.VksSelectionLawunit.JudgeDepartmentRoleId == NomenclatureConstants.JudgeDepartmentRole.Predsedatel)
                        {
                            ...
                            //Председателите излизат Последни (IV ти докладчик)
                    ...
                    foreach (var month_lawunit in selestion_month.SelectionMonthLawunit.OrderBy(x => x.VksSelectionLawunit.OrderNumber))
                    {
                        ...
                        {
                            shortName = month_lawunit.VksSelectionLawunit.LawUnit.FamilyName + month_lawunit.VksSelectionLawunit.LawUnit.Family2Name;
                        }
                        if (judgeCount < 3)
                        {
        ...
        Boolean IsSelectionStaffSameAsPrevious(VksSelection selection, int selectionMonth)
        {
            ...
            return (repo.AllReadonly<VksSelectionMonth>().Where(x => x.VksSelectionId == selection.Id).Where(x => x.SelectionMonth == selectionMonth).Where(x => prevStafF.Contains(x.SelectionHash)).Any());
        ...
        public Boolean ReplacedLawunitSave(VksSelectionLawunit model)
        {
            ...
            try
            {
                ...
                var replaceLawunitWithRole = repo.AllReadonly<CourtDepartmentLawUnit>()
                                               ...
                                               .Where(x => x.CourtDepartmentId == selection.CourtDepartmentId || x.CourtDepartment.ParentId == selection.CourtDepartmentId).FirstOrDefault();
                ...
                replacing.CourtDepartmentTypeId = replaceLawunitWithRole.CourtDepartment.DepartmentTypeId;
        ...
        public List<VksSessionDayCalendarVM> GetvksSessionDayCalendar(int[] LawunitsInCalendar, int? courtDepartmentId = 0)
        {
            ...
            var sessionDayCalendar = repo.All<VksSelectionMonth>()
                             ...
                             .Where(x => x.SelectionMonthLawunit.Where(y => selection_lawUnits.Contains(y.VksSelectionLawunitId)).Any())
...

            {
                foreach (var lau_unit in selection_month.LawUnitsList.OrderBy(x => x.LawUnitOrder))
                {
                    ...
                    lu_cases = all_cases.Where(x => !x.CaseSessions.Any())
        ...
        public bool SelectionHasUnfilledSessionDates(int selectionId)
        {
            bool result = false;
            try
            {
                result = repo.AllReadonly<VksSelectionMonth>()
                             .Where(x => x.VksSelectionId == selectionId)
                             .Where(x => x.SessionDate == null).Any();
