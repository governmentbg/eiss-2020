...
namespace IOWebApplication.Core.Contracts
{
    public interface IEisppService : IBaseService
    {
        ...
        List<SelectListItem> GetDDL_EISPPEventType(int caseCodeId, bool isExternal, int eventTypeId, bool addDefaultElement = true);
        ...
        EisppDropDownVM GetDDL_FeatureValTblElementWithRules(int eventType, string rulePath, List<SelectListItem> ek_countries);
        ...
        Task<EisppUnionVM> GenerateUnionPackage(int eventTypeId, int casePrincipalId, int caseAddedId, int caseSessionActId);
        List<SelectListItem> CaseAddedDDL(int caseId);
        EisppUnionVM GetPackageUnion(int packageId);
        EisppBaseCase GetConnectedCase(int caseId, int eventType, string connectedCaseId);
        EisppEventGSVM InitEisppEventGSVM(Event eisppEvent);
        void EisppEventGSVM_To_Event(EisppEventGSVM model, Event eisppEvent);
        EisppEventGSVM GetPackageEventGS(int packageId);
        bool HaveEventCreateCase(int caseId);
