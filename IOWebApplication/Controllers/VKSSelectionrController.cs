...
namespace IOWebApplication.Controllers
{
    public class VKSSelectionController : BaseController
    {
        ...
        public IActionResult PreviewEDIT(int id)
        {
            ...
            if (commonService.CheckCourtRestriction(NomenclatureConstants.CourtRestrictionTypes.VKSPreviewEdit))
            {
                return RedirectToAction(nameof(HomeController.AccessDenied), HomeController.ControlerName);
            }
            ViewBag.HasSignedProtocol = vksSelectionService.SelectionHasSignedProtocol(id);
        ...
        public IActionResult SendForSign(int Id)
        {
            ...
            if (vksSelectionService.SelectionHasUnfilledSessionDates(protocol.VksSelectionId))
            {
                SetErrorMessage("Протокол не може да бъде подписан доката има непопълнени дати на заседания!");
                return RedirectToAction("ProtocolList", new { id = protocol.VksSelectionId });
            }
